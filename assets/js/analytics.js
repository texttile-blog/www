/* ====================================================================
   ...
   The theme rides along on every event as a registered property, so a
   ?theme= visit can be told apart afterwards.
   ==================================================================== */
...
(function () {
  ...
  var theme = m && THEMES.indexOf(m[1].toLowerCase()) >= 0 ? m[1].toLowerCase() : 'elixir';

  /* on every event from here on, so the numbers can be read per theme
     and a ?theme= link can be told from a plain visit */
  posthog.register({ theme: theme, themed_url: !!m });
